...
object TerminationRequests extends ServicesConfiguration {
  ...
  def terminationBody(): String =
    s"""
       |{
       |  "nationalInsuranceNumber": "$nationalInsuranceNumber",
       |  "universalCreditRecordType": "UC",
       |  "universalCreditAction": "Terminate",
       |  "liabilityStartDate": "2025-02-19",
       |  "liabilityEndDate": "2025-08-19"
       |}
       |""".stripMargin

  val termination: HttpRequestBuilder =
    http("Send termination notification")
      ...
      .body(StringBody(terminationBody()))
      .check(status.is(204))
}
